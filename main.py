...
p3={'name':'Bhuvaneswar Kumar','role':'bowl','wkts':1,'overs':10,'runs':71,'field':1}
...
from cricket_pts import batscore,bowlscore

for p in players:
    if p['role']=='bat':
        score=batscore (p['runs'],p['4'],p['6'],p['balls'],p['field'])
    else:
        score=bowlscore (p['wkts'],p['runs'],p['overs'],p['field'])
